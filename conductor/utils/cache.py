...
class CompilationCache:
    ...
    def __init__(self, cache_dir: Optional[str] = None, max_size_mb: Optional[int] = None):
        ...
        Args:
            cache_dir: Directory for cache storage (default: from config or system temp)
            max_size_mb: Maximum cache size in megabytes (default: from config)
        """
        # Get configuration
        try:
            ...
            if cache_dir is None:
                cache_dir = cache_config.cache_dir or os.path.join(
                    tempfile.gettempdir(), "conductor_cache"
                )
    ...
    def get(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to load cached artifact (likely due to module structure change): {e}"
            )
    ...
    def _save_metadata(self) -> None:
        ...
        try:
            ...
            metadata_with_version = {"cache_version": CACHE_VERSION, "entries": self._metadata}
                ...
                with open(self._metadata_file, "rb") as f:
                    ...
                    if isinstance(data, dict) and "cache_version" in data:
                        ...
                        if stored_version != CACHE_VERSION:
                            logger.info(
                                f"Cache version mismatch (stored: {stored_version}, current: {CACHE_VERSION}). Clearing cache."
                            )
                            self.clear()
                    else:
    ...
    def _enforce_size_limit(self) -> None:
        ...
        sorted_entries = sorted(self._metadata.items(), key=lambda x: x[1]["last_accessed"])
