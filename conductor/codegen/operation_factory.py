...
class OperationFactory:
    ...
    def create_conductor_node(self, fx_node: fx.Node, metadata: Optional[dict[str, Any]] = None) -> ConductorNode:
    ...
    def _generate_simple_dsl(self, node: ConductorNode, **kwargs) -> list[str]:
        ...
        if not op_desc:
            return [f"{output_var}[{index_vars}] = {input_vars[0]}[{index_vars}]; // Unknown: {node.op_name}"]
        ...
        if op_desc.operation_type.value == "elementwise":
            if node.op_name in op_symbols:
                if node.op_name == "relu":
                    return [f"{output_var}[{index_vars}] = {op_symbols[node.op_name].format(input_vars[0] + '[' + index_vars + ']')};"]
                elif len(input_vars) >= 2:
                    return [f"{output_var}[{index_vars}] = {input_vars[0]}[{index_vars}] {op_symbols[node.op_name]} {input_vars[1]}[{index_vars}];"]
                else:
                    return [f"{output_var}[{index_vars}] = {input_vars[0]}[{index_vars}]; // {node.op_name} (single input)"]
            else:
                return [f"{output_var}[{index_vars}] = {node.op_name}({input_vars[0]}[{index_vars}]);"]
            ...
            if node.op_name in ["matmul", "mm", "bmm"]:
                ...
                return [f"{output_var}[{index_vars}] = {node.op_name}({input_vars[0]}[{index_vars}]); // matrix"]

        elif op_desc.operation_type.value == "reduction":
            return [f"{output_var}[0] = {node.op_name}({input_vars[0]}[{index_vars}]); // reduction"]

        else:
            return [f"{output_var}[{index_vars}] = {node.op_name}({input_vars[0]}[{index_vars}]); // {op_desc.operation_type.value}"]
    
    def is_supported_operation(self, op_name: str) -> bool:
        """Check if an operation is supported."""
        return (get_operator_template(op_name) is not None or 
                get_op_desc(op_name) is not None)
...
def create_conductor_node(fx_node: fx.Node, metadata: Optional[dict[str, Any]] = None) -> ConductorNode:
    """Create a ConductorNode from an FX node."""
