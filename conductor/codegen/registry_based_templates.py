...
from .operator_registry import operator_registry, get_operator_template, OperatorInfo as OperatorTemplate
...
class TemplateContext:
    ...
    dimensions: dict[str, int] = field(default_factory=lambda: {
        'M': 16, 'N': 16, 'P': 4, 'buffer_m': 16, 'buffer_n': 8
    })
...
class ComputationExtractor:
    """Simple computation extractor for operator fusion."""

    def extract_computation(self, template: OperatorTemplate) -> str:
        """Extract computation from template or use simple mapping."""
        # Simple mapping for common operations
        computations = {
            'add': 'output[i] = input0[i] + input1[i];',
            'mul': 'output[i] = input0[i] * input1[i];',
            'sub': 'output[i] = input0[i] - input1[i];',
            'div': 'output[i] = input0[i] / input1[i];',
            'relu': 'output[i] = max(0.0f, input[i]);'
        }

        return computations.get(template.name, f'output[i] = {template.name}(input[i]);')
...
class RegistryBasedTemplateEngine:
    ...
    def __init__(self):
        self.computation_extractor = ComputationExtractor()
    ...
    def render_fused_operations(self, nodes: list[ConductorNode], context: TemplateContext) -> str:
        ...
        if base_template and base_template.code_template:
            ...
            return f"// Fused operations: {', '.join(n.op_name for n in nodes)}\n{fused_computation}"
...
def render_node_with_registry_templates(node: "ConductorNode", function_name: str = "generated_function") -> str:
...
def render_fused_nodes_with_registry_templates(nodes: list[ConductorNode], function_name: str = "generated_function") -> str:
...
def initialize_registry_templates():
    ...
    logger.info(f"Registry-based template engine initialized with {len(available_ops)} operations: {available_ops}")
