...
class DebugTracer:
        ...
        if True:  # Always show metadata when debug is enabled
            ...
            for i, tensor in enumerate(inputs):
                ...
                if tensor.numel() <= self.config.max_tensor_elements:
                    ...
                    print(f"  Data: {tensor.flatten()[:self.config.max_tensor_elements]}... (truncated)")
                print()
            ...
            for key, value in node.metadata.items():
                try:
                    if isinstance(value, (dict, list)):
                        print(f"    {key}: {json.dumps(value, indent=self.config.indent_size, default=str)}")
                    else:
                        print(f"    {key}: {value}")
            ...
            for j, other_node in enumerate(dag.nodes):
                if i != j:
                    ...
                    if node_outputs.intersection(other_inputs):
                        connections[f"Node_{i}_{node.op_name}"].append(f"Node_{j}_{other_node.op_name}")
    ...
    def print_host_wrapper_details(self, wrapper_code: str, function_signature: str, 
...
def trace_host_wrapper(wrapper_code: str, function_signature: str, 
                      buffer_info: Dict[str, Any]) -> None:
