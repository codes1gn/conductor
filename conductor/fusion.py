...
if TYPE_CHECKING:
    from .graph_analyzer import ConductorNode, ComputationDAG

# Import unified operator system for fusion decisions
try:
    from .operator_registry import get_operator_template, get_fusion_metadata, can_operators_fuse
...
class FusionCluster:
    ...
    def validate_fusion_safety(self) -> bool:
        ...
        for i, node in enumerate(self.nodes):
            for j, other_node in enumerate(self.nodes[i+1:], i+1):
    ...
    def generate_fused_dsl(self) -> str:
        ...
        function_signature = f"function {self.dsl_function_name}({', '.join(input_names)}) -> ({', '.join(output_names)})"
    ...
    def estimate_performance_gain(self) -> float:
        ...
        total_gain = 1.0 + kernel_launch_savings + memory_locality_gain + type_gain - complexity_penalty
...
class FusionEngine:
    ...
    def _find_elementwise_chain(self, start_node: 'ConductorNode', dag: 'ComputationDAG', visited: set) -> List['ConductorNode']:
        ...
        while True:
            ...
            if len(current.outputs) == 1:
                ...
                elementwise_consumers = [
                    consumer for consumer in output_buffer.consumers
                    if (consumer not in visited and 
                        self._is_elementwise_op(consumer.op_name) and
                        len(consumer.inputs) == 1)  # Single input for simple chaining
                ]
    ...
    def _find_elementwise_producers(self, reduction_node: 'ConductorNode', dag: 'ComputationDAG', visited: set) -> List['ConductorNode']:
    ...
    def _find_buffer_reuse_opportunities(self, lifetimes: dict) -> dict:
        ...
        for i, buffer1 in enumerate(buffers):
            for j, buffer2 in enumerate(buffers[i+1:], i+1):
                if (buffer1.dtype == buffer2.dtype and 
                    buffer1.shape == buffer2.shape and
                    self._lifetimes_non_overlapping(lifetimes[buffer1], lifetimes[buffer2])):
    ...
    def _lifetimes_non_overlapping(self, lifetime1: dict, lifetime2: dict) -> bool:
        ...
        return (lifetime1['end'] < lifetime2['start'] or 
    ...
    def apply_elementwise_fusion(self, nodes: List['ConductorNode']) -> FusionCluster:
        ...
        cluster = FusionCluster(
            ...
            dsl_function_name=function_name
        )
