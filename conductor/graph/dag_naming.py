...
from ..codegen.dsl_constants import (
    DSLIdentifierGenerator, MemoryLevel, DSLGenerationContext, ContextKeys
)
...
class DAGNamingAnnotator:
    ...
    def _annotate_node(self, node: "ConductorNode", index: int, annotation: DAGNamingAnnotation) -> NodeNaming:
        ...
        if op_info and op_info.input_buffers:
            ...
            # Fallback
            var_name = annotation.identifier_generator.generate_load_var("input", naming.memory_level)
            naming.input_vars.append(var_name)
            naming.load_vars.append(var_name)
    ...
    def _ensure_naming_consistency(self, annotation: DAGNamingAnnotation, sorted_nodes: list["ConductorNode"]):
