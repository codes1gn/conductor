...
from ..utils.trace import get_debug_tracer, trace_host_wrapper
...
class ChoreoHostInterface:
    ...
    def generate_host_wrapper_code(
        self,
        function_name: str,
        input_descriptors: list[ChoreoTensorDescriptor],
        output_descriptor: ChoreoTensorDescriptor,
    ) -> str:
        """
...
extern choreo::spanned_data<choreo::{output_descriptor.dtype}, {output_descriptor.rank}> {function_name}("""
        ...
        for i, desc in enumerate(input_descriptors):
            param_strs.append(
                f"const choreo::spanned_view<choreo::{desc.dtype}, {desc.rank}>& input_{i}"
            )
...
"""
    ...
    def compile_host_wrapper(
        ...
        try:
            ...
            raise DeviceError(
                "Compiler not found. Please ensure topscc or g++ is installed and in PATH."
            )

        # Note: We no longer delete the wrapper file - it's saved in debug directory for inspection

    def create_output_tensor(
        self, descriptor: ChoreoTensorDescriptor, device: str = "cpu"
    ) -> torch.Tensor:
        """
    ...
    def execute_kernel_with_library(
        self, lib: ctypes.CDLL, input_tensors: list[torch.Tensor], output_tensor: torch.Tensor
    ) -> int:
        """
