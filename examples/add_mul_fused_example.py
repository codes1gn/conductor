#!/usr/bin/env python3
"""
Fused Add-Multiply Example with GCU Acceleration

This example demonstrates how to use torch.compile with the GCU backend
for fused element-wise operations (addition followed by multiplication).
It shows the complete end-to-end workflow and demonstrates kernel fusion.

Key Features:
- Fused element-wise operations (add + multiply)
- torch.compile integration with GCU backend
- Kernel fusion optimization demonstration
- Performance comparison between CPU and GCU
- Numerical correctness verification
- Debug artifact inspection

Usage:
    python3 add_mul_fused_example.py
"""
...
class FusedAddMulModel(nn.Module):
    """
    Model that performs fused element-wise addition and multiplication.
    
    This model demonstrates kernel fusion where multiple element-wise
    operations are combined into a single GCU kernel for better performance.
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        """
        Perform fused addition and multiplication.
        ...
        Returns:
            Result of (x + y) * z
        """
        # This should be fused into a single kernel by the GCU backend
        intermediate = x + y  # Addition
        result = intermediate * z  # Multiplication
        return result


def run_cpu_baseline(model: nn.Module, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> tuple:
...
def run_gcu_accelerated(model: nn.Module, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> tuple:
...
def inspect_debug_artifacts():
    ...
    if os.path.exists(debug_dir):
        ...
        # Look for DSL files to check fusion
        dsl_files = [f for f in os.listdir(debug_dir) if f.endswith('.choreo')]
        if dsl_files:
            print(f"   💡 Check {dsl_files[0]} to see if operations were fused")
    else:
        print(f"📁 No debug artifacts found (directory {debug_dir} doesn't exist)")


def main():
    """Main function demonstrating fused add-multiply with GCU acceleration."""
    print("Fused Add-Multiply Example with GCU Acceleration")
    print("=" * 60)
    ...
    print(f"  Operation: fused add-multiply ((x + y) * z)")
    print(f"  Expected: kernel fusion optimization")
    print()
    ...
    z = torch.randn(batch_size, feature_size, dtype=torch.float32)
    
    print(f"✅ Model created: FusedAddMulModel")
    print(f"✅ Input tensors: x{x.shape}, y{y.shape}, z{z.shape}")
    print()
    
    # Run CPU baseline
    print("🖥️  Running CPU baseline...")
    try:
        cpu_output, cpu_time = run_cpu_baseline(model, x, y, z)
    ...
    try:
        gcu_output, gcu_time, compile_time = run_gcu_accelerated(model, x, y, z)
    ...
    if gcu_time > 0:
        ...
        if speedup > 1.0:
            print(f"🎉 GCU acceleration achieved!")
            print(f"💡 Kernel fusion may have contributed to performance gains")
        else:
            print(f"💡 GCU overhead present (expected for small tensors)")
    ...
    # Final status
    if is_correct:
        print("🎯 Success! Fused add-multiply with GCU acceleration working correctly.")
        print()
        print("💡 Key takeaways:")
        print("   - Fused operations successfully compiled for GCU")
        print("   - Kernel fusion optimization demonstrated")
        print("   - Numerical correctness maintained")
        print("   - Debug artifacts available for fusion inspection")
        return True
    else:
